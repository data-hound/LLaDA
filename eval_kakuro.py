#!/usr/bin/env python3
"""
eval_kakuro.py  —  Evaluate LLaDA-8B-Instruct on kakuro (NineGrid) benchmarks.
...
────────────────────────────────
  ...
  python eval_kakuro.py --parquet ninegrid.parquet --few-shot 0 --batch-size 64

  # 5-shot
  python eval_kakuro.py --parquet ninegrid.parquet --few-shot 5 --batch-size 64
...
try:
    ...
    sys.exit(
        f"[eval_kakuro] Cannot import generate.py: {exc}\n"
        "Place eval_kakuro.py in the same directory as generate.py and try again."
    )
...
def load_model_and_tokenizer(model_path: str, device: str):
    print(f"[eval_kakuro] Loading model: {model_path}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Evaluate LLaDA-8B-Instruct on Apaxdor Kakuro_mania (via HuggingFace).",
